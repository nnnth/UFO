...
backend_args = None
pretrained = 'https://download.openmmlab.com/mmclassification/v1/vit_sam/vit-large-p16_sam-pre_3rdparty_sa1b-1024px_20230411-595feafd.pth'
...
model = dict(
    ...
    backbone=dict(
        ...
        drop_path_rate=0.4,
        mean_layers=[24,25,26,27,28,29],
        init_cfg=dict(type='Pretrained', checkpoint=pretrained, prefix='backbone.'),
        text_cfg=dict(type='bert-base', hidden_size=1024, 
                      pretrain_path='./ckpt/bert_embed_large.pt',vocab_size=30525),),
    head_list=dict(
...
train_dataloader = dict(
    ...
    sampler=dict(type='GroupMultiSourceNonMixedSampler', batch_size=3, source_ratio=[0.2, 0.2, 0.2, 0.2, 
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'backbone.layers.13': dict(lr_mult=0.175),
            'backbone.layers.14': dict(lr_mult=0.25),
            'backbone.layers.15': dict(lr_mult=0.325),
            'backbone.layers.16': dict(lr_mult=0.4),
            'backbone.layers.17': dict(lr_mult=0.475),
            'backbone.layers.18': dict(lr_mult=0.55),
            'backbone.layers.19': dict(lr_mult=0.625),
            'backbone.layers.20': dict(lr_mult=0.7),
            'backbone.layers.21': dict(lr_mult=0.775),
            'backbone.layers.22': dict(lr_mult=0.85),
            'backbone.layers.23': dict(lr_mult=0.925),
            'backbone.layers.24': dict(lr_mult=1.0),
            'backbone.layers.25': dict(lr_mult=1.0),
            'backbone.layers.26': dict(lr_mult=1.0),
            'backbone.layers.27': dict(lr_mult=1.0),
            'backbone.layers.28': dict(lr_mult=1.0),
            'backbone.layers.29': dict(lr_mult=1.0),
        }))
