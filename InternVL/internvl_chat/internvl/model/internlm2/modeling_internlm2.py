...
def get_vl_attn_mask(input_embed, attn_mask, selected):
    B, N  = input_embed.shape[:2]
    num_image_token = int((selected.sum() / B).item())
    ...
    bidr_mask = selected[:, None, None, :].expand(-1, 1, num_image_token, -1)
...
class InternLM2Model(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        image_selected: torch.BoolTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        if self.config.attn_implementation == 'flash_attention_2':
            ...
            if image_selected.shape[-1] == attention_mask.shape[-1]:
                attention_mask = get_vl_attn_mask(inputs_embeds, (attention_mask<-1), image_selected)
        ...
        model_inputs.update(
            {
                ...
                'attention_mask': attention_mask,
                'image_selected': kwargs.get('image_selected'),
            }
        )
        return model_inputs
