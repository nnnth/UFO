...
class RefCOCODataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'text': self.prompt.format(text.lower()),
    ...
    for ds_name in args.datasets:
        ...
        if torch.distributed.get_rank() == 0:
            ...
            for i, output in enumerate(merged_outputs):
                predict_bbox = [output['answer'].split(',')]
                try:
                    predict_bbox = (float(predict_bbox[0][0]), float(predict_bbox[0][1]), float(predict_bbox[0][2]),
                                    float(predict_bbox[0][3]))
                    cx, cy, w, h = predict_bbox[0], predict_bbox[1], predict_bbox[2], predict_bbox[3]
                    predict_bbox = ((cx-0.5*w), (cy-0.5*h), (cx+0.5*w), (cy+0.5*h))
...
if __name__ == '__main__':
    ...
    prompt = 'Where can we locate {} in the image?'
