...
if [ ${DATASET} == "refcoco" ]; then
    ...
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/refcoco/evaluate_grounding_ufo.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi
...
if [ ${DATASET} == "refcoco-val" ]; then
    ...
    --nproc_per_node=${GPUS} \
    --master_port=${MASTER_PORT} \
    eval/refcoco/evaluate_grounding_ufo.py --checkpoint ${CHECKPOINT} --datasets refcoco_val "${ARGS[@]:2}"
fi
