...
class DetInferencer(BaseInferencer):
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Compose:
        ...
        load_referseg_idx = self._get_transform_idx(pipeline_cfg,
                                               'LoadAnnotationsReferSeg')
        if load_referseg_idx != -1:
            del pipeline_cfg[load_referseg_idx]

        load_reasonseg_idx = self._get_transform_idx(pipeline_cfg,
                                               'LoadAnnotationsReasonSeg')
        if load_reasonseg_idx != -1:
            del pipeline_cfg[load_reasonseg_idx]
    ...
    def preprocess(self, inputs: InputsType, text: str = None, is_sentence: bool = None, batch_size: int = 1, **kwargs):
        ...
        """
        chunked_data = self._get_chunk_data(inputs, batch_size, text, is_sentence)
        yield from map(self.collate_fn, chunked_data)

    def _get_chunk_data(self, inputs: Iterable, chunk_size: int, text: str = None, is_sentence : bool = None):
            ...
            try:
                ...
                for _ in range(chunk_size):
                    ...
                    if text is not None:
                        ...
                        pipeline_input = inputs_
                    if is_sentence is not None:
                        assert text is not None
                        pipeline_input['is_sentence'] = is_sentence
                    chunk_data.append((inputs_, self.pipeline(pipeline_input)))
                yield chunk_data
    ...
    def __call__(self,
        ...
        inputs = self.preprocess(
            ori_inputs, text=text, is_sentence=is_sentence, batch_size=batch_size, **preprocess_kwargs)
        if text is not None:
            self.visualizer.dataset_meta['classes'] = [text[:20]]
    ...
    def visualize(self,
        ...
        for single_input, pred in zip(inputs, preds):
            ...
            if 'pred_instances' not in pred:
                ...
                if 'pred_bboxes' in pred:
                    pred_instances.bboxes = pred.pred_bboxes
                else:
                    pred_instances.masks = pred.pred_sem_seg.data.bool()
                    pred_instances.bboxes = torch.zeros((1,4))
                pred_instances.scores = torch.ones((1,))
                pred_instances.labels = torch.tensor([0])
                pred.pred_instances = pred_instances
