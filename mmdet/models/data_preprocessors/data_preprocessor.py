...
class GeneralDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        """
        batch_pad_shape = self._get_pad_shape(data)
        if data['inputs'][0].ndim == 4:
            # split caption to seperate data samples
            new_data = {
                'inputs':[],
                'data_samples':[]
            }
            for i in range(len(data['inputs'])):
                multi_inputs = data['inputs'][i]
                multi_num = len(multi_inputs)
                gt_captions = deepcopy(data['data_samples'][i].gt_caption)
                for j in range(multi_num):
                    new_data['inputs'].append(multi_inputs[j])
                    new_data_sample = deepcopy(data['data_samples'][0])
                    new_data_sample.gt_caption = gt_captions[j]
                    new_data['data_samples'].append(new_data_sample)

                    # import cv2 
                    # cv2.imwrite(f'{i*16+j}_{gt_captions[j]}.jpg',new_data['inputs'][-1].cpu().numpy().transpose(1,2,0))
            data = new_data
        # else:
        #     import cv2
        #     cv2.imwrite(f'detection.jpg',data['inputs'][0].cpu().numpy().transpose(1,2,0))
        #     exit()
