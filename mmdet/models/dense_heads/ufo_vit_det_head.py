...
class UFOViTDetHead(BaseModule):
    ...
    def get_prompt(self, batch_data_samples):
        ...
        prompt = f'Task: {self.task_prompt}; Class: {classes_text}; Bin: 0 to {num_bins}'
    ...
    def get_prompt_sample(self, batch_data_samples, target_tokens):
        ...
        for target_token in target_tokens:
            # remove background
            pos_classes = target_token[:,0].unique()[:-1]
        ...
        for targets_tokens in targets_tokens_tensor:
            ...
            for tokens in targets_tokens:
                if use_vocab_list[tokens[0]] == 'background':
                    ...
                    text = ','.join([use_vocab_list[idx] for idx in tokens])
    ...
    def loss_by_feat_single(self, pred_seq_logits: Tensor, 
        ...
        loss_cls = self.loss_reg(pred_seq_logits, targets_tokens_tensor) / avg_factor.to(torch.float32)
        return (loss_cls,)
    ...
    def _predict_single(self, logits: Tensor, ids: Tensor, refer: Tensor,
        ...
        for k, text in enumerate(topk_texts):
            splits = text.split(',')
            if splits[0] != 'background' and splits[0] in class_names and len(splits) >= 5:
                det_labels.append(class_names_dict[splits[0]])
                det_scores.append(scores[k])
                try: 
                    bboxes = torch.FloatTensor([int(num.replace(' ',''))/num_bins for num in splits[1:5]]).to(topk_refer.device)
