...
class UFOLLaVAInsSegHead(BaseModule):
    ...
    def get_prompt(self, batch_data_samples):
        ...
        classes = self.dataset_labels[dataset_name]
        if isinstance(classes, dict):
            classes = list(classes.values())
            new_classes = []
            for cls_text in classes:
                if isinstance(cls_text, tuple):
                    cls_text = cls_text[0] + " " + cls_text[1]
                new_classes.append(cls_text)
            classes = new_classes
        else:
            classes = list(classes)
        num_classes = batch_data_samples[0].metainfo['head_cfg']['num_classes']
        classes_text = ','.join(classes)
        prompt = f"A chat between a curious human and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the human's questions. User: \nSegment objects in the image that belong to the following categories: {classes_text}.\n<image> ASSISTANT: "
    ...
    def get_prompt_sample(self, batch_data_samples, target_tokens):
        ...
        if isinstance(classes, dict):
            classes = list(classes.values())
            # determine use " " or the " " of the ""
            self.combine_random = random.random()
        else:
            classes = list(classes)
        num_classes = batch_data_samples[0].metainfo['head_cfg']['num_classes']
        prompts = []
        for target_token in target_tokens:
            ...
            # replace tuple with text 
            new_sample_classes = []
            for s_c in sample_classes_list:
                if isinstance(s_c, tuple):
                    if self.combine_random < 0.5:
                        s_c = s_c[0] + " " + s_c[1]
                    else:
                        s_c = "the {} of the {}".format(s_c[1], s_c[0])
                new_sample_classes.append(s_c)
            sample_classes_list = new_sample_classes
        ...
        for targets_tokens in targets_tokens_tensor:
            ...
            for tokens in targets_tokens:
                if use_vocab_list[tokens[0]] == 'background':
                    ...
                    cls_text = use_vocab_list[tokens[0]] 
                    if isinstance(cls_text, tuple):
                        if self.combine_random < 0.5:
                            cls_text = cls_text[0] + " " + cls_text[1]
                        else:
                            cls_text = "the {} of the {}".format(cls_text[1], cls_text[0])
                    text = cls_text + ','+'<MASK>'*16
    ...
    def loss_by_feat_single(self, pred_seq_logits: Tensor, 
        ...
        for k, gt_instance in enumerate(batch_gt_instances):
            ...
            # pred_mask = mask_features @ single_image_features.flatten(0,1).permute(1,0) / mask_features.shape[-1]
    ...
    def _predict_single(self, logits: Tensor, ids: Tensor, refer: Tensor, img_feat, output_feat,
        ...
        if len(mask_feat) > 0:
            ...
            # pred_mask = mask_feat @ img_feat.flatten(0,1).permute(1,0) / mask_feat.shape[-1]
