...
class HungarianTwiceMaskAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        assigned_gt_inds = torch.full((num_preds*max_pred, ),
                                      -1,
                                      dtype=torch.long,
                                      device=device)
        assigned_labels = torch.full((num_preds*max_pred, ),
                                     -1,
                                     dtype=torch.long,
                                     device=device)
        ...
        # maximum cost in first assign
        first_cost_max = cost[matched_row_inds,matched_col_inds].max()
        ...
        # second assign
        cost[matched_row_inds,matched_col_inds] = 1e5
        matched_row_inds, matched_col_inds = linear_sum_assignment(cost)
        # twice assign cost < first max
        twice_cost = cost[matched_row_inds, matched_col_inds]
        twice_mask = twice_cost < first_cost_max
        if twice_mask.any():
            matched_row_inds = matched_row_inds[twice_mask]
            matched_col_inds = matched_col_inds[twice_mask]
            
            matched_row_inds = torch.from_numpy(matched_row_inds).to(device) + num_preds
            matched_col_inds = torch.from_numpy(matched_col_inds).to(device)
            assigned_gt_inds[matched_row_inds] = matched_col_inds + 1
            assigned_labels[matched_row_inds] = gt_labels[matched_col_inds]
        return AssignResult(
