...
@TASK_UTILS.register_module()
class PointsL1Cost(BaseMatchCost):
    """PointsL1Cost.
    ...
    def __call__(self,
        ...
        Args:
            pred_instances (:obj:`InstanceData`): ``points`` inside is
                predicted points with normalized coordinate
                (cx, cy).
            gt_instances (:obj:`InstanceData`): ``bboxes`` inside is gt
                bboxes with unnormalized coordinate (x, y, x, y).
            img_meta (Optional[dict]): Image information. Defaults to None.
        ...
        if self.box_format == 'xywh':
            ...
            gt_points = torch.cat([gt_bboxes[:, 0] + gt_bboxes[:, 2] / 2, 
                                    gt_bboxes[:, 1] + gt_bboxes[:, 3] / 2], dim=1)
...
@TASK_UTILS.register_module()
class CentroidCost(BaseMatchCost):
    """PointsL1Cost.
...
class MaskCost(BaseMatchCost):
    ...
    def __call__(self,
        ...
        long_points = pred_points.long()
        point_masks = gt_masks[:,long_points[:,1],long_points[:,0]]

        mask_cost = 1-point_masks.permute(1,0)
        
        return mask_cost * self.weight

@TASK_UTILS.register_module()
class MaskRandomCost(BaseMatchCost):
    """PointsL1Cost.
    ...
    def __call__(self,
        ...
        mask_cost = mask_cost * self.weight + torch.rand(mask_cost.shape,device=mask_cost.device)
