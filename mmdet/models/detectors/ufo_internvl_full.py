...
class UFO_InternVL_Full(BaseDetector, metaclass=ABCMeta):
    r"""
    Args:
    ...
    def __init__(self,
        ...
        init.ones_(self.backbone.language_model.model.norm.weight)

        if pretrain_path is not None:
            self.load_state_dict(torch.load(pretrain_path,map_location=self.backbone.device))
            print(f'load {pretrain_path}')
    ...
    def forward_transformer(self, patch_embed: Tensor, patch_resolution: Tuple,
        ...
        if self.mode != 'vqa':
            ...
            if len(assistant_pos) != len(token_weights):
                ...
                img_end_token = self.tokenizer.convert_tokens_to_ids('</img>')
        ...
        if self.training:
            ...
            position_ids = None
            for layer_id, layer in enumerate(self.backbone.language_model.model.layers): 
                input_embed = checkpoint.checkpoint(layer.__call__, input_embed, attn_mask, position_ids=position_ids, use_reentrant=False)[0]

            input_embed = self.backbone.language_model.model.norm(input_embed)
            ...
            for token_idx in range(29):
                ...
                seq_embed = self.backbone.language_model.model.norm(input_embed)
    ...
    def grid_forward_transformer(self, patch_embed: Tensor, patch_resolution: Tuple,
        ...
        if self.training:
            ...
            softmax_logits = logits.softmax(-1)
            if self.repeat_grid and self.mode in ['detection','instance_segmentation']:
                bg_id = self.tokenizer.convert_tokens_to_ids('background')
                softmax_logits[:, :, bg_id] = 0
                logits, input_ids = softmax_logits.topk(self.repeat_num)
                logits = logits.flatten(1,2)
                input_ids = input_ids.flatten(1,2)

                # filter top k sub-sequences
                topk_logits, topk_ids = logits.topk(self.keep_num)
                input_ids = torch.gather(input_ids, 1, topk_ids)
                logits = topk_logits
            

                # repeat kv cache
                new_kv_caches = {}
                for layer_id in range(len(self.backbone.language_model.model.layers)):
                    ori_kv_cache = torch.stack(kv_caches[layer_id],dim=0)
                    observe_kv, grid_kv = ori_kv_cache[:, :, :, :-grid_num], ori_kv_cache[:, :, :, -grid_num:]
                    repeat_grid_kv = grid_kv.unsqueeze(4).repeat(1,1,1,1,self.repeat_num,1).flatten(3,4)
                    repeat_grid_kv = torch.gather(repeat_grid_kv, 3, topk_ids[None, 
                                        :, None, :, None].repeat(2, 1, repeat_grid_kv.shape[2], 1, repeat_grid_kv.shape[-1]))
                    repeat_kv = torch.cat([observe_kv, repeat_grid_kv],dim=3)
                    new_kv_caches[layer_id] = (repeat_kv[0],repeat_kv[1])
                del kv_caches
                kv_caches = new_kv_caches

                grid_num = logits.shape[1]
            else:
                logits, input_ids = softmax_logits.max(dim=-1)
            ...
            if self.repeat_grid and self.mode in ['detection','instance_segmentation']:
                 # repeat then filter
                global_grid_reference = global_grid_reference.unsqueeze(2).repeat(1,1,3,1).flatten(1,2)
                global_grid_reference = torch.gather(global_grid_reference, 1, topk_ids.unsqueeze(-1).repeat(1,1,2))
