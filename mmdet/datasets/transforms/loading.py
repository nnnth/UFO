...
from mmdet.utils import datafrombytes
import cv2
from mmdet.datasets.data_processing import get_mask_from_json
import random 
...
@TRANSFORMS.register_module()
class LoadAnnotationsReferSeg(MMCV_LoadAnnotations):
    """Load and process the ``instances`` and ``seg_map`` annotation provided
    by dataset.
    ...
    def __init__(self,
                 **kwargs) -> None:
        super(LoadAnnotationsReferSeg, self).__init__(**kwargs)
    ...
    def _load_masks(self, results: dict) -> None:
        ...
        Args:
            results (dict): Result dict from :obj:``mmengine.BaseDataset``.
        """
        raw_mask = results['mask']
        if len(raw_mask) == 0:
            m = np.zeros((results["height"], results["width"])).astype(
                np.uint8
            )
        else:
            if type(raw_mask[0]) == list:  # polygon
                rle = maskUtils.frPyObjects(
                    raw_mask, results["height"], results["width"]
                )
            else:
                rle = raw_mask
        ...
        del results['mask']
        h, w = results['ori_shape']
        results['gt_masks'] = BitmapMasks([m],h,w)
...
@TRANSFORMS.register_module()
class LoadAnnotationsReasonSeg(MMCV_LoadAnnotations):
    def __init__(self,
                ignore=False,
                 **kwargs) -> None:
        self.ignore = ignore
        super(LoadAnnotationsReasonSeg, self).__init__(**kwargs)
    ...
    def _load_masks(self, results: dict) -> None:
        ...
        """
        h, w = results['ori_shape']
        mask, sents, is_sentence = get_mask_from_json(results['mask_path'], h, w)

        # NOTE: set 255 to 0, check better way to ignore
        if self.ignore:
            mask[mask==255] = 0
...
@TRANSFORMS.register_module()
class LoadAnnotationsReasonDet(MMCV_LoadAnnotations):
    def __init__(self,
                ignore=False,
                 **kwargs) -> None:
        self.ignore = ignore
        super(LoadAnnotationsReasonDet, self).__init__(**kwargs)
    ...
    def mask2box(self,mask):
        ...
        # 返回边界框的 (x_min, y_min, x_max, y_max)
    ...
    def _load_bboxes(self, results: dict) -> None:
        ...
        if self.ignore:
            mask[mask==255] = 0
        bbox = self.mask2box(mask)
        if bbox is None:
            bbox = [0,0,1,1]
        results['gt_bboxes'] = np.array(bbox,dtype=np.float32)[None,:]
...
@TRANSFORMS.register_module()
class SegLoadAnnotations(MMCV_LoadAnnotations):
