    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        if results.get('gt_seg_map', None) is not None:
            results['gt_seg_map'] = results['gt_seg_map'][crop_y1:crop_y2,
                                                          crop_x1:crop_x2]
        if results.get('gt_bboxes', None) is None and results.get('gt_masks', None) is not None:
...
class AddMetaInfo(BaseTransform):
    """Add Task name to the inputs data.
    """
    ...
    def transform(self, results: dict) -> dict:
        if self.meta_dict is not None:
            for k in list(self.meta_dict.keys()):
                results[k] = self.meta_dict[k]        
